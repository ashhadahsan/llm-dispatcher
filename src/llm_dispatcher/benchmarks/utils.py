...
def export_benchmark_data(
    results: List[Dict[str, Any]], filepath: Union[str, Path], format: str = "json"
) -> None:
    """
...
def validate_test_cases(test_cases: List[Dict[str, Any]]) -> Tuple[bool, List[str]]:
    ...
    for i, test_case in enumerate(test_cases):
        ...
        if "prompt" not in test_case:
            errors.append(f"test_cases[{i}] missing required field: prompt")
        elif (
            not isinstance(test_case["prompt"], str) or not test_case["prompt"].strip()
        ):
            errors.append(f"test_cases[{i}] prompt must be a non-empty string")

        # Check evaluator or expected_output
        if "evaluator" not in test_case and "expected_output" not in test_case:
            errors.append(
                f"test_cases[{i}] must have either evaluator or expected_output"
            )

        # Validate evaluator if present
        if "evaluator" in test_case and not callable(test_case["evaluator"]):
            errors.append(f"test_cases[{i}] evaluator must be callable")

        # Validate expected_output if present
        if "expected_output" in test_case and not isinstance(
            test_case["expected_output"], str
        ):
            errors.append(f"test_cases[{i}] expected_output must be a string")

    return len(errors) == 0, errors


def validate_test_prompts(test_prompts: List[str]) -> Tuple[bool, List[str]]:
    """
    ...
    for i, prompt in enumerate(test_prompts):
        if not isinstance(prompt, str):
            errors.append(f"test_prompts[{i}] must be a string")
        elif not prompt.strip():
            errors.append(f"test_prompts[{i}] cannot be empty")
        elif len(prompt) > 10000:  # Reasonable limit
            errors.append(f"test_prompts[{i}] is too long (max 10000 characters)")
